    ...
    return (
        <div className="flex justify-center font-[Iosevka] text-white w-full h-screen content-center bg-zinc-950 p-5">
            <div className="flex flex-col justify-center gap-2 h-full items-center">
                ...
                <div className="w-fit h-fit px-8 md:px-15 pt-6 md:pt-10 pb-3 md:pb-8 border border-white/80 bg-white/10 rounded-4xl md:w-lg">
                    <h1 className="text-4xl md:text-5xl text-white/95 font-semibold text-center mb-5 md:mb-8">Create Account</h1>
                    <form className="flex flex-col gap-3 justify-center place-center mb-2 md:mb-4 items-center" onSubmit={handleSubmit}>
                        <input className="md:w-xs lg:w-sm md:h-12 text-lg md:text-xl px-2 md:px-3 py-1 md:py-2 border border-white/50 bg-white/10 rounded-md placeholder:text-white/50 focus:outline-none focus:ring-1" type="text" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} />
                        ...
                        <button className="self-center md:text-xl px-4 md:px-8 py-1 md:py-2 mt-1 md:mt-3 border border-white/80 bg-white/20 border-2 rounded-full w-fit text-center hover:bg-white/40" type="submit">Register</button>
                    </form>
                    <h3 className="text-center md:text-lg">Already have an Account? <a href="/login" className="text-slate-300 hover:underline underline-offset-3">Login-here</a></h3>
                </div>
            </div>
        </div>
    )
